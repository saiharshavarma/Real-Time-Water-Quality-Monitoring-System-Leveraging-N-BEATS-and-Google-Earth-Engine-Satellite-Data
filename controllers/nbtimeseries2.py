...
pred_c = model_nb.predict(n=2, series=series_c)
...
print(list(last_5_days.tail(1).index)[0].strftime("%Y-%m-%d"))
print(statistics.median(list(last_5_days['C'])))

# d = datetime.now().strftime("%Y-%m-%d")
# # print(df.index.max())
# d2 = datetime.now() + relativedelta(days=7)
# future = pd.date_range(d,d2, freq='7d')
# # print(future)
...
for x in list(pred_c_inv.pd_dataframe().index):
  print(x.strftime("%Y-%m-%d"),end=" ")
print()
