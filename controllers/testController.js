...
const getReflectanceLandsat = async (req, res) => {
  ...
  var ptsLandsatStats = zonalStats(oliCol, ptsLandsat, params).then(
    async (result) => {
      ...
      // toSaveNonSplit.forEach(element => {
      ...
      for (let index = 1; index < 6; index++) {
        ...
        console.log(data1[data1.length - index]);
        let options = {
            mode: "text",
        ...
        const {
            ...
            // "Producing Code" (May take some time)
            PythonShell.run("Rrs.py", options, async function (err, results) {
              if (err) {
                console.log(err);
                myReject({ success: false, err: err });
              }
              // results is an array consisting of messages collected during execution
              myResolve({
                success: true,
      ...
      const {
        ...
        // "Producing Code" (May take some time)
        PythonShell.run("predict3.py", options, async function (err, results) {
      ...
      if (success) {
        ...
        res.json({
          ...
          data: {
            fui_alpha: alpha,
            satellite_data: datum0,
            predicted_chl: results,
            calculated_vars: {b443:res_array[0][0],w:res_array[0][1],R443:res_array[0][2],r443:res_array[0][3],a443:res_array[0][4],aw443:res_array[0][5],bw443:res_array[0][6], adg_443:res_array[0][7], bbp_443: res_array[0][8]},
          },
        });
      } else {
        // console.log("Test Error: " + err);
